...
class InfererURL():
    ...
    def run(self):
        # pred_map = self.__gen_prediction(self.input_img)
        # np.save(f"{name_out}_map.npy", pred_map) # {'result':[pred_map]}
        proc_result = self.__process()
        temp_file = NamedTemporaryFile()
        out = os.path.join(self.save_dir, os.path.split(temp_file.name)[0])
        name_out = os.path.join(self.save_dir, os.path.split(temp_file.name)[1])
        
        cv2.imwrite(f'{self.save_dir}/{name_out}.png', proc_result[0])
        print(f"Saved pred to <{name_out}.npy>. {datetime.now().strftime('%H:%M:%S.%f')}")
        np.save(f'{self.save_dir}/{name_out}.npy', proc_result[1])
        print(f"Saved processed image to <{np_out}.png>. {datetime.now().strftime('%H:%M:%S.%f')}")
